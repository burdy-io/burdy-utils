// @ts-nocheck
import { forEach, isEmptyString } from './utility';
...
function getEntityMarkup(entityMap, entityKey, text, customEntityTransform) {
  ...
  if (entity.type === 'IMAGE') {
    ...
    components.push(`<img src='${src}' ${getAlt(alt)} width='${width}' height='${height}' />`);
    if (caption?.length > 0) {
  ...
  if (entity.type === 'TEXT_EDITOR') {
    ...
    return `<pre><code class="language-${mode}">${value}</code></pre>`;
  }
 ...
 */
export function getBlockInnerMarkup(block, entityMap, hashtagConfig, customEntityTransform) {
  const blockMarkup = [];
 ...
 */
export function getBlockMarkup(block, entityMap, hashtagConfig, directional, customEntityTransform) {
  const blockHtml = [];
 ...
 */
const getCodeMarkup = (codeBlocks, entityMap, hashtagConfig, directional, customEntityTransform?) => {
  const codeHtml = [];
  let nestedCodeBlock = [];
  let previousBlock;
  codeBlocks.forEach((block) => {
    ...
    if (!previousBlock) {
      ...
      if (nestedCodeBlock && nestedCodeBlock.length > 0) {
        codeHtml.push(getListMarkup(nestedCodeBlock, entityMap, hashtagConfig, directional, customEntityTransform));
        nestedCodeBlock = [];
      }
    } else {
      nestedBlock = true;
...
export const richtextToHtml = (editorContent: any, hashtagConfig?, directional?, customEntityTransform?) => {
  const html = [];
  if (editorContent) {
